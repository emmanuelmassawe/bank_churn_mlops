...
class ChurnPredictor {
  ...
  collectFormData() {
    ...
    for (const [apiKey, elementId] of Object.entries(formElements)) {
      ...
      if (!value) {
        throw new Error(
          `Please fill in the ${element.labels[0]?.textContent || elementId} field`,
        );
      }
  ...
  hideLoading() {
    ...
    this.predictBtn.innerHTML =
      '<i class="fas fa-brain"></i> Analyze Customer Risk';
  }
  ...
  async checkAPIHealth() {
    try {
      ...
      this.showNotification(
        "🔴 API Not Available - Please start your FastAPI server",
        "error",
      );
    }
  }
